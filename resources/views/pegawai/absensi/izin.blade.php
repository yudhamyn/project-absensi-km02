                        ...
                        <div class="card-body">
                            <?php if ($detail_absensi->izin == null) : ?>
                                ...
                                <form action="{{ url('') }}/pegawai/izin" method="post" enctype="multipart/form-data">
                                    @csrf
                                    <!-- Field wrapper start -->
                                    <input type="hidden" name="kode_absensi" value="<?= $detail_absensi->kode_absensi; ?>">
                            ...
                            <?php else : ?>
                                ...
                                <div class="doc-block" style="padding: 13px;">
                                    ...
                                    <a href="{{ url('') }}/pegawai/download_izin/<?= $detail_absensi->bukti_izin; ?>" class="btn btn-primary" target="_blank">Download</a>
                                </div>
                                <p>Status Izin : <?= ($detail_absensi->status_izin == 0) ? '<span class="badge bg-warning">Pending</span>' : (($detail_absensi->status_izin == 1) ? '<span class="badge bg-success">Di Setujui</span>' : '<span class="badge bg-danger">Ditolak</span>'); ?></p>
                                <a href="{{ url('') }}/pegawai/absensi" class="btn btn-danger mt-2"><i class="icon-arrow-left"></i>Kembali</a>
                            <?php endif; ?>
